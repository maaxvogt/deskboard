...
final class ClaudeStatusClient {
    ...
    func refetch() async {
        let settings = AppSettings.shared
        let base = settings.claudeAPIBase.trimmingCharacters(in: .whitespaces)
        guard !base.isEmpty else {
            error = "Set the claude-status API URL in Settings"
            return
        }
        guard !settings.claudeToken.isEmpty, let url = URL(string: base + "/api/tasks?filter=all") else {
    ...
    private func runSocket() async {
        let settings = AppSettings.shared
        guard !settings.claudeToken.isEmpty, !settings.claudeAPIBase.isEmpty else {
