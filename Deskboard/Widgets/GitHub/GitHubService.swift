...
final class GitHubService {
    ...
    func refresh() async {
        if Demo.active {
            configured = true
            openPRs = Demo.pullRequests
            events = Demo.githubEvents
            error = nil
            return
        }
    ...
    private func fetchOpenPRs(
        ...
        // Usernames are [A-Za-z0-9-], but never trust settings input in a URL.
        let safeUser = user.addingPercentEncoding(withAllowedCharacters: .alphanumerics) ?? ""
        let query = "is:pr+is:open+author:\(safeUser)"
    ...
    private func fetchEvents(user: String, token: String) async throws -> [GitHubEvent] {
        let (data, response) = try await URLSession.shared.data(
            for: request("/users/\(user.addingPercentEncoding(withAllowedCharacters: .alphanumerics) ?? "")/events?per_page=15", token: token))
        try check(response)
